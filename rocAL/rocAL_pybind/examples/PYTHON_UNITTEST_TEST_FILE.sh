...
do 
    ...
    do 
        ...
        # resize
        python"$ver" rocAL_api_python_unittest.py --image-dataset-path "$image_path" --augmentation-name resize --batch-size $batch_size  --max-width $width --max-height $height --color-format $rgb --resize-interpolation-type 1 --resize-scaling-mode 0 --$backend_arg -f "${output_path}Resize_${rgb_name[$rgb]}_${device_name}_bilinear_default_FileReader"
        ...
        python"$ver" rocAL_api_python_unittest.py --image-dataset-path "$caffe_detection_path" --reader-type "caffe_detection" --augmentation-name resize --batch-size $batch_size  --max-width $width --max-height $height --color-format $rgb --resize-interpolation-type 0 --resize-scaling-mode 0 --$backend_arg -f "${output_path}Resize_${rgb_name[$rgb]}_${device_name}_nearestneighbor_default_caffeDetection"
