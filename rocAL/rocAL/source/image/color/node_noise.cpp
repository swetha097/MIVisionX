...
void NoiseTensorNode::create_node()
{
    ...
    _node = vxExtrppNode_Noise(_graph->get(), _inputs[0]->handle(), _src_tensor_roi, _outputs[0]->handle(), _alpha.default_array(), _beta.default_array(), _hue.default_array(), _sat.default_array(),seed, layout, roi_type, _batch_size);
...
void NoiseTensorNode::init( float alpha, float beta, float hue , float sat,int seed)
{
...
void NoiseTensorNode::init( FloatParam* alpha, FloatParam* beta, FloatParam* hue, FloatParam* sat, int seed)
{
